...
class userService {
    ...
    async getdata(userToken) {
        ...
        const userTokenData = await tokenModel.findOne({userToken});
        if(!userTokenData) 
    ...
    async updateUserData(userData) {
        const user = await UserModel.findOne(userData.email);
        if(!user)
            throw ApiError.UnauthorizedError();
        ...
        const hashPassword = await bcrypt.hash(userData.newPassword, 3);
        const userUpdatedInfo = await UserModel.updateOne({name: userData.name, email: userData.email, password: hashPassword});
        ...
        const commonDto = new CommonDto(updateResponse);

        return { updateStatus: commonDto };
    }

    async subscription(email) {
        const candidate = await OrderModel.findOne({email});
        if(candidate)
            throw ApiError.BadRequest(`User with email: ${email} already subscribe!`);
